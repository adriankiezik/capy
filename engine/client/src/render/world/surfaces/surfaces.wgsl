...
@vertex fn vs_main(@location(0) position: vec3<f32>, @location(1) normal: vec3<f32>, @location(2) color: vec3<f32>, @location(3) translation: vec4<f32>, @location(4) occlusion: f32, @location(5) origin: vec4<f32>) -> Output {
    var out: Output;
...
@fragment fn fs_main(input: Output) -> @location(0) vec4<f32> {
    ...
    let light = camera.sky_ambient.w * mix(0.35, 1.0, input.occlusion) + (1.0 - camera.sky_ambient.w) * direct;
