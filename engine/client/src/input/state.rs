...
impl ButtonBinding {
    ...
    pub(crate) fn consume(&self, input: &mut Input) {
...
impl AxisBinding {
    ...
    pub(crate) fn buttons(&self) -> impl Iterator<Item = Button> + '_ {
        self.negative
            .buttons
            .iter()
...
impl Axis2Binding {
    ...
    pub(crate) fn enabled(&self, input: &Input) -> bool {
        !self.captured_only || input.cursor_captured
    }

    pub(crate) fn uses_mouse_motion(&self) -> bool {
...
pub struct Input {
    down: HashSet<Button>,
    pub(crate) initial_down: HashSet<Button>,
...
impl Input {
    ...
    pub(crate) fn button_state(&self, buttons: &[Button]) -> ButtonState {
    ...
    pub(crate) fn hide(&mut self, button: Button) {
        self.initial_down.remove(&button);

        self.changes.retain(|&(changed, _)| changed != button);
    }

    pub(crate) fn suppress_held(&mut self, button: Button) -> bool {
        self.initial_down.remove(&button);
    ...
    pub(crate) fn consume_until_release(&mut self, button: Button) {
        self.hide(button);
    ...
    pub(crate) fn take_captured_mouse_delta(&mut self) -> Vec2 {
        let delta = if self.focused && self.cursor_captured {
            self.captured_mouse_delta
        } else {
            Vec2::ZERO
        };
    ...
    pub(crate) fn discard_mouse_motion(&mut self) {
    ...
    pub(crate) fn sync_cursor(&mut self, captured: bool, revision: u64) {
    ...
    pub(crate) fn motion(&mut self, delta: (f64, f64)) {
        if self.focused {
    ...
    pub(crate) fn change(&mut self, button: impl Into<Button>, down: bool) {
        let button = button.into();
