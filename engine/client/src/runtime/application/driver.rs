...
pub fn run<F: Future<Output = ApplicationResult<()>> + 'static>(
    ...
    if !(1..=1000).contains(&settings.runtime.input_rate)
        || settings.runtime.max_inputs_per_update == 0
    {
    ...
    let clock = InputClock::new(
        settings.runtime.input_rate,
        settings.runtime.max_inputs_per_update,
    );
...
struct InputClock {
    tick: Duration,
    max_inputs_per_update: u32,
    last_update: Option<Instant>,
    accumulated: Duration,
}

impl InputClock {
    fn new(input_rate: u32, max_inputs_per_update: u32) -> Self {
        Self {
            tick: Duration::from_secs_f64(1.0 / input_rate as f64),
            max_inputs_per_update,
            last_update: None,
    ...
    fn advance(&mut self, now: Instant) -> u32 {
        ...
        due.min(self.max_inputs_per_update as u128) as u32
    }

    fn next_input(&self) -> Option<Instant> {
        self.last_update
...
impl Shared {
    pub(super) fn update_window_controls(&mut self) -> Result<()> {
        let session = self.session.as_mut().ok_or(RuntimeError::NotRunning)?;

        self.input.sync_cursor(
        ...
        let result = session
            .window_controls
            .update(&mut self.input, &mut session.window);
    ...
    F: Future<Output = ApplicationResult<()>>,
{
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if let Some(deadline) = self.next_redraw.filter(|&t| t > Instant::now()) {
            ...
            window.request_redraw();
        } else if let Some(deadline) = self.clock.next_input().filter(|&t| t > Instant::now()) {
            event_loop.set_control_flow(ControlFlow::WaitUntil(deadline));
        } else {
            window.request_redraw();
        }
