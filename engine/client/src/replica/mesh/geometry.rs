...
impl MeshSource {
    ...
    pub(crate) fn resolve<'a>(
        ...
        pollster::block_on(build_halo(key, edge, scratch, world, maximum)).map(Arc::new)
    }
}
...
pub(crate) fn build(
    ...
    pollster::block_on(build_halo(key, edge, scratch, world, maximum))
}
    ...
    for axis in 0..3 {
        for sign in [-1, 1] {
            for slice in 0..edge {
                ...
                fill_mask(mask, edge, origin, axis, sign, slice, halo);
