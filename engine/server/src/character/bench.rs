...
use super::{Character, CharacterConfig, CharacterPose, MovementInput};
use crate::{
    simulation::{PhysicsConfig, Simulation, SimulationConfig},
...
fn scene(movement: Movement) -> Simulation {
...
fn advance(bencher: Bencher, sprint: bool, movement: Movement) {
    ...
    let settings = CharacterConfig::default();
    ...
    if !matches!(movement, Movement::Free) {
        player
            .advance(&scene, MovementInput::default(), delta)
