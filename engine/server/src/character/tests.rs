...
fn player(settings: CharacterConfig) -> Character {
    Character::new(
        settings,
        CharacterPose {
...
fn obstacle_course_blocks_walls_and_ceiling_then_allows_another_grounded_jump() {
    ...
    let mut player = player(CharacterConfig::default());

    let dt = Duration::from_secs_f32(1.0 / 60.0);

    for _ in 0..5 {
        player
            .advance(&scene, MovementInput::default(), dt)
            .unwrap();
    }
...
fn diagonal_speed_is_normalized_and_failed_partial_movement_rolls_back() {
    ...
    let mut straight = player(CharacterConfig::default());
    ...
    assert_eq!(limited.direction(), control.direction());

    for _ in 0..20 {
        limited
            .advance(&scene, MovementInput::default(), Duration::from_millis(10))
            .unwrap();
