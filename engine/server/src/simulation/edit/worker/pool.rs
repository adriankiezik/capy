...
use crate::{
    simulation::{Result, SimulationError, connectivity::Connectivity},
    world::VoxelCoord,
};
use std::{
    ...
    fn poll(self: &Arc<Self>, cache: &mut Connectivity) {
        ...
        if !self.cancelled.load(Ordering::Acquire) {
            ...
            match prepared {
                ...
                Err(_) => {
                    let _ = self.result.send(Err(SimulationError::EditWorkerStopped));
                }
            }
        }
...
pub(in crate::simulation::edit) struct Receipt {
    task: Arc<Task>,
    receiver: Option<mpsc::Receiver<Result<Patch>>>,
}

impl Receipt {
    pub(in crate::simulation::edit) fn try_recv(
        &self,
...
pub(in crate::simulation::edit) struct Workers {
    queue: Arc<Shared>,
    capacity: usize,
...
impl Workers {
    pub(in crate::simulation::edit) fn new(count: usize) -> Result<Self> {
        ...
        for index in 0..count {
            ...
            let handle = std::thread::Builder::new()
                ...
                .map_err(SimulationError::EditWorker)?;
    ...
    pub(in crate::simulation::edit) fn submit(
        &self,
        input: Input,
        voxels: Vec<VoxelCoord>,
        structural: bool,
        ...
        if queue.closing {
            return Err(SimulationError::EditWorkerStopped);
        }
    ...
    pub(in crate::simulation::edit) fn retire(&self, patch: Patch) {
        self.queue
            .0
            .lock()
            .unwrap_or_else(|error| error.into_inner())
