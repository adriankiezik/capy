...
fn capacity_recovers_after_cancellation() {
    ...
    let scene = crate::simulation::Simulation::new(crate::simulation::tests::settings()).unwrap();

    let input = || Input::capture(&scene, crate::simulation::edit::Domain::Static).unwrap();
...
fn duplicate_wakes_complete_once_and_panics_do_not_stop_workers() {
    for count in [1, 4] {
        ...
        assert!(matches!(
            receive(&failed),
            Err(SimulationError::EditWorkerStopped)
        ));
