...
fn dynamic_scene(length: i32, protrusion: bool) -> Simulation {
...
fn finish(scene: &mut Simulation, edits: &mut SimulationEdits) -> Vec<EditOutcome> {
...
fn unrelated_commit_preserves_in_flight_dynamic_edit() {
    ...
    let mut edits = SimulationEdits::new(&scene, EditSettings::default()).unwrap();
...
#[test]
fn local_removal_publishes_geometry_and_empty_removal_is_unchanged() {
    ...
    let cold = dynamic_scene(16, false);
...
proptest::proptest! {
    ...
    fn contact_index_matches_transitive_full_scan(
        ...
        let slop = fixture.settings.physics.contact_slop;
...
fn scheduler_bounds_pending_work_and_retries_conflicting_edits() {
    ...
    let mut edits = SimulationEdits::new(
        &scene,
        EditSettings {
            workers: 4,
            max_pending: 2,
