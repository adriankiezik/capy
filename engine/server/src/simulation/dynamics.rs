use crate::{
    Aabb,
    simulation::{Body, PhysicsConfig, Result, SimulationError, body::BodyGeometry},
    ...
    for y in (min_voxel_y..=max_voxel_y).rev() {
        *probe_count += 1;
        ...
        if *probe_count > settings.max_collision_probes {
            return Err(SimulationError::Limit("dynamic collision probes"));
        }
...
fn body_allowed_drop(
    ...
    for y in (column.min_voxel_y..=column.max_voxel_y).rev() {
        *probe_count += 1;
        ...
        if *probe_count > settings.max_collision_probes {
            return Err(SimulationError::Limit("dynamic pair probes"));
        }
