...
use super::{EditSettings, PhysicsConfig, Simulation, SimulationConfig, SimulationEdits, Target};
use crate::{
    Aabb,
    world::{
        Material, MaterialId, Owner, OwnerId, StructureId, Transaction, VOXEL_SIZE, Voxel,
        VoxelBounds, WorldSettings,
    },
};
...
fn place(scene: &mut Simulation, positions: impl IntoIterator<Item = IVec3>, owner: u64) {
...
fn query_scene(distance: i32) -> Simulation {
    let mut scene = scene();
...
fn edit_fixture(count: i32, stride: i32) -> (Simulation, Option<Transaction>) {
    let scene = scene();
...
fn supported_beam(length: i32) -> (Simulation, Option<Transaction>) {
    let mut scene = scene();
...
fn falling_scene(count: i32) -> Simulation {
    let mut scene = scene();
...
fn finish_edits(scene: &mut Simulation, edits: &mut SimulationEdits) {
...
fn edit_bodies(bencher: Bencher, count: i32, workers: usize) {
    ...
    let run = |scene: &mut Simulation, edits: &mut SimulationEdits| {
    ...
    bencher
        ...
        .with_inputs(|| {
            ...
            let edits = SimulationEdits::new(&scene, settings).expect("valid scheduler settings");
...
fn edits_scale(
    ...
    let mut fixture = Simulation::new(settings).expect("valid scale settings");
