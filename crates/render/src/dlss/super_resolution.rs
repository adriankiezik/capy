...
impl DlssSuperResolution {
    ...
    pub fn render(
        ...
        // Get the raw VkImage for the output texture (vk::Image is Copy).
        ...
        unsafe {
            ...
            dlss_command_encoder.as_hal_mut::<Vulkan, _, _>(|command_encoder| {
                ...
                // The wgpu Transit barrier transitions the output texture to
                // VK_IMAGE_LAYOUT_GENERAL with SHADER access, but the DLSS SDK
                // internally clears it via vkCmdClearColorImage which requires
                // TRANSFER_WRITE at the TRANSFER stage. Insert a barrier to make
                // the Transit's layout-transition write visible to the clear and
                // subsequent shader operations.
                let image_barrier = ash::vk::ImageMemoryBarrier::default()
