use crate::world::{LEAF_EDGE, Leaf, Voxel, VoxelCoord};
use glam::IVec3;
...
impl Halo {
    ...
    pub(super) fn new(origin: IVec3, edge: i32, sample: impl Fn(VoxelCoord) -> Voxel) -> Self {
    ...
    pub(super) fn voxel(&self, p: VoxelCoord) -> Voxel {
        let p = (p - self.origin).as_uvec3();

        self.voxels[p.x as usize + self.edge * (p.y as usize + self.edge * p.z as usize)]
    }

    pub(super) fn occlusion(&self, p: VoxelCoord, axis: usize, sign: i32) -> [u8; 4] {
